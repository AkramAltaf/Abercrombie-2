...
function getAddedTaskList() {
  var addedTaskItems = [];
  let storedTaskItems = localStorage.getItem("addedTaskItems");

  if (!!storedTaskItems) {
    addedTaskItems = [...JSON.parse(storedTaskItems), listItemName];
    for (let tasks of addedTaskItems) {
      console.log(tasks);
    }
  } else {
    addedTaskItems.push(listItemName);
  }
  return addedTaskItems;
}

var addTask = function () {
  listItemName = taskInput.value;
  ...
  if (listItemName.trim() !== "") {
    ...
    taskInput.value = "";
    localStorage.setItem("addedTaskItems", JSON.stringify(getAddedTaskList()));
  } else {
